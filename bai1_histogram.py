...
def iris_dataset(histogram=False, boxplot=True, pie=False, scatter2d=False, matrix=False, parallel=False):
    ...
    iris_features_class = ["sepal_length", "sepal_width", "petal_length", "petal_width", "class"]
    df = pd.read_csv(iris_path, sep=',', names=iris_features_class)
    ...
    if parallel:
        ...
        #         if (iris_classes[i] == class_name[j]):
        #             iris_classes_id.append(j)
        # iris_classes_id_transpose = np.array(iris_classes_id)
        # iris_data = np.column_stack((iris_data, iris_classes_id_transpose))

        # iris_df = pd.DataFrame(data=iris_data, columns=iris_features + ["class_id"])
        # iris_df.head()

        # print(iris_df)
        # #iris_features_class.append('class_id')
        # #print(iris_features_class)
        # fig = px.parallel_coordinates(iris_df, dimensions=iris_features,
        #                               color="class_id", range_color=[0, 2],
